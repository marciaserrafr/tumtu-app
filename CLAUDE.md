...
- **Dimensão da carteirinha é fixa em 300×500px (documentado em `docs/tumtu-design-guide.md`, seção 7.3) — nunca aumentar o tamanho do cartão pra caber conteúdo novo.** Se fonte, logo, respiro ou qualquer elemento não couber, o ajuste é no conteúdo (reduzir/redistribuir), nunca no tamanho do cartão. Mudar essa dimensão exige aprovação explícita da Márcia todas as vezes — regra criada em 13/jul/2026 depois de eu aumentar o tamanho sem avisar duas vezes seguidas na mesma sessão.
...
🚧 **Redesign da carteirinha digital em andamento (13/jul/2026)** — só em arquivos de teste locais ainda (`preview-carteirinha.html` + `styles/carteirinha-tumtu-novo.css`, nenhum commitado, `carteirinha.html` real intocado). Regras completas em `docs/design-guide-atualizacao-carteirinha.md`: sistema de cores por N cores da escola (calculado via `color-mix()` a partir de 1-4 cores reais, testado com Grupo Especial + Série Ouro + o caso de 4 cores da União de Maricá), rodapé da frente com cor real mas sem virar bloco opaco, verso na cor cheia da escola, dimensão fixa 300×500px (nunca mudar sem aprovação — ver regra abaixo), especificação de arquivo de logo documentada (PNG transparente, recortado, quadrado, mín. 300×300px) mas ainda sem o textinho de instrução dentro do produto. Campo de gênero já existe no banco (ver acima) — falta só aplicar o rótulo Mestre/Mestra no CSS novo quando o redesign for pra produção.
