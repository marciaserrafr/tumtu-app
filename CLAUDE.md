...
🚧 Pendências conhecidas (não urgentes): ver seção 9 de `tumtu-documentacao-tecnica.md` — "Leaked Password Protection" adiada por depender de plano pago do Supabase. Apagar a tabela antiga `ritmistas` (só depois da Márcia testar o site publicado). Sistema de nomenclatura de tamanho de roupa por escola (XXG vs XGG), adiado a pedido dela. Aba "Bateria" do Super Admin tem um campo antigo "Mestre de Bateria" (texto livre, desconectado do sistema real de vínculos) que confunde — Márcia pediu pra ajustar, sem pressa.
🚧 **Redesign visual "cara de app moderno" — descoberta feita, desenho ainda não começado (16/jul/2026).** Escopo do piloto: `login.html`, `cadastro.html`, "Meu Perfil" (`ficha-perfil`) e a tela de carregando — tirar de vez o padrão "modal/dialog" (caixa branca flutuando em fundo escuro) e virar tela cheia de verdade. Admin/Super Admin ficam pra uma fase futura (ela já avisou que quer estender pra lá também, mas não agora). Referências dela: Instagram, Disney+, Netflix, Spotify, Nubank, Google/Gmail, Uber — decisão já fechada de ir no caminho escuro de ponta a ponta (Spotify/Netflix/Disney+), campos de formulário também escuros (`--cor-fundo-medio: #1e1b2e`) com borda clara, não campos brancos. Detalhe completo da entrevista de UX e das referências: `tumtu-documentacao-tecnica.md` seção 26.
...
**Sequência combinada em 14–16/jul/2026, específica da carteirinha/piloto (prioridade dela, fora do roadmap de 10/jul acima):** 1) cor/logo por escola ✅ concluído 15/jul → 2) botões novos da carteirinha (Meu Perfil + Trocar de Bateria) ✅ concluído 16/jul → 3) redesign visual de login/cadastro/perfil pra "cara de app moderno" 🚧 descoberta feita, desenho ainda não começado → 4) configuração de medidas (mesma lógica dos instrumentos) — ainda não começado. Fluxo de login pós-piloto completo pro Admin (cair direto em `admin.html`) fica pra **depois do piloto estar redondo**, decisão explícita dela em 16/jul — "o piloto é o piloto".
