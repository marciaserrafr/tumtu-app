...
> Última atualização: 31/ago/2026, sessão seguinte (Convidado Especial: fase de tela quase pronta no preview, bloqueada por bug crítico de duplicação; pendência nova de Figurino)
...
✅ **O que já está em produção, funcionando de ponta a ponta**: autenticação real + RLS, motor único de edição de perfil, cadastro em etapas com autocadastro/link fixo/manual, carteirinha digital com cor/logo dinâmicos por escola, aprovação de Ritmistas/Diretoria com histórico de auditoria, permissões granulares por pessoa (Reforma de Permissões, 28/ago/2026), Instrumentos/Medidas/Figurino como bibliotecas mestre 100% configuráveis, exportação para Excel, PWA instalável, Face ID/Digital como trava local de privacidade dentro do app, módulo de Presença via QR Code (leitor embutido no app, tanto pra Diretoria escanear quanto pra Ritmista "Confirmar Presença" sozinho), domínios `tumtu.com.br` e `app.tumtu.com.br` no ar, fundamentos de LGPD (política de privacidade, exclusão de dados sob pedido). Painel de gestão é um único arquivo (`admin.html`) com barra própria pro Super Admin. Auditorias de segurança recorrentes já corrigiram vazamentos reais várias vezes — ver regra "Segurança" acima.

🔜 **Próxima pendência a atacar: "Convidado Especial" — FASE DE TELA quase pronta (branch `preview/convidado-especial`, 31/ago/2026), mas BLOQUEADA por um bug crítico de perda/duplicação de dado, ainda sem diagnóstico.** Gente de fora da bateria oficial que a Diretoria quer com carteirinha de verdade (login) — diferente do "Convidado" (`extras`) que já existe, sem login. Testado ao vivo nesta sessão: cadastrar 2+ pessoas DIFERENTES (CPF/e-mail diferentes) como Convidado Especial na mesma bateria faz só a última sobreviver na fila — as anteriores somem, mesmo sendo pessoas de verdade distintas no banco. Suspeita (não confirmada): efeito colateral do gatilho que trava `vinculos.eh_convidado` depois de criado. **Precisa de acesso ao Supabase pra diagnosticar — não subir pra produção antes disso.** **Achado importante da sessão: o link de preview usa o MESMO banco de produção** — qualquer cadastro de teste feito por ele grava dado real, visível em produção, porque o código de produção não sabe filtrar `eh_convidado`. Isso já causou um vazamento real (pessoas de teste "Teste"/"Convidado Apoio" apareceram misturadas em Ritmistas/Diretoria/Permissões da Imperatriz de verdade) — corrigido com uma contenção emergencial já publicada direto na `main` (filtro `eh_convidado=eq.false` nas 3 listas), mas os registros de teste em si só saem do banco se ela excluir manualmente pela tela Privacidade (conferir se já fez isso). Resto da fase de tela (menu "Convidados" roteando sozinho entre modelo Simples/Especial conforme Comercial, fila, permissões separadas por cargo — Convidado-Ritmista/Diretor de Bateria/Apoio, Medida reaproveitando o checkbox "Convidados" de Categoria de Figurino, Naipe travado pra sempre) está pronto e testado no preview. Detalhe completo: `docs/tumtu-historico-sessoes.md` (última entrada) e memória de longo prazo `project_convidado_com_carteirinha`.

🔜 **Pendência nova (31/ago/2026, acionada por ela testando Convidado Especial): Figurino (`figurino_itens_mestre`) tem "Público" e "Incluir Convidados" no lugar errado.** Hoje essas duas decisões moram no item GLOBAL (Super Admin → Configurações → Figurino, vale pra todas as escolas) — ela apontou que isso está errado, porque só cada bateria sabe quem vai receber uma peça (ex: "Camisa da Final") e se quer incluir Convidados nela. Devia funcionar igual Categoria de Figurino/Medida já funciona: item global só define nome + qual Categoria usa pro tamanho; "Público" e "Incluir Convidados" viram decisão por bateria (mover de `figurino_itens_mestre` pra `bateria_figurino_itens`). Concordei que faz sentido — mudança de banco (precisa de acesso ao Supabase), ainda não desenhada em detalhe nem implementada.
