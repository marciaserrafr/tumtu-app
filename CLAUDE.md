...
🚧 Pendências conhecidas (não urgentes, todas confirmadas como pós-piloto em 17/jul/2026): "Leaked Password Protection" (depende de plano pago do Supabase). Apagar a tabela antiga `ritmistas` (risco confirmado baixo — só 2 das 28 linhas são reais, já migradas corretamente; falta só o "pode apagar" final dela). Aba "Bateria" do Super Admin tem um campo antigo "Mestre de Bateria" (texto livre, desconectado do sistema real de vínculos) que confunde. Face ID/sessão persistente, cadastro de temporadas com histórico, interruptor pra ritmista editar as próprias medidas — 3 ideias registradas, nenhuma desenhada ainda.
