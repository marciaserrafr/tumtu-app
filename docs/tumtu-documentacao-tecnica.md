...
### 68.7 Terceira rodada (mesma sessão, pedido dela: "quer fazer uma terceira revisão, por favor?") — foco em coluna sem trava, achando o gap mais sensível das três rodadas

`get_advisors` rodado de novo primeiro (confirmando zero regressão das duas correções anteriores), depois uma checagem que nenhuma das duas rodadas anteriores tinha feito: enumerar toda RLS policy de `UPDATE` que não checa capacidade nenhuma (só "é admin dessa bateria/escola?"), e, pra cada uma, ler o trigger `aplicar_matriz_edicao_X` correspondente coluna por coluna contra a lista real de colunas da tabela (`information_schema.columns`) — não confiar que "o trigger existe" significa "o trigger cobre tudo".

**Achado 1 (baixo risco)**: `escolas.nome_curto` (campo criado em 23/ago/2026) nunca entrou em `aplicar_matriz_edicao_escolas()` — corrigido, agora exige `editar_dados_escola` como o resto dos campos de identidade da escola.

**Achado 2 (o mais sensível das três rodadas de hoje)**: `pessoas.responsavel_nome`/`responsavel_cpf`/`responsavel_celular` (dados do responsável de um menor, seção 39) nunca entraram em `aplicar_matriz_edicao_pessoas()`, em NENHUMA das duas ramificações (nem autoedição, nem edição por terceiro) — diferente de `nascimento`/`cpf`, que já seguiam a regra dela ("dado pessoal ngm pode editar mesmo... só a própria pessoa, ou eu") nas duas ramificações desde sempre. A tela nunca ofereceu esses campos como editáveis pra ninguém além de Super Admin, mas isso era só a tela — o banco aceitava a mudança via API direta. `pessoas.qr_token` (nunca escrito por nenhum código do app, confirmado por grep, sempre gerado sozinho via `gen_random_uuid()`) tinha o mesmo problema — sem essa trava, qualquer pessoa com UPDATE na própria linha (ou Diretoria da própria bateria, na linha de um Ritmista) conseguiria "plantar" um `qr_token` escolhido e depois consultar os dados de emergência dessa pessoa à vontade pela função pública `buscar_emergencia_por_qr` (seção 68.1) — reabrindo por um caminho diferente o mesmo tipo de problema que a criação do `qr_token` tinha resolvido. `foto_pos_x`/`foto_pos_y` (posição do recorte da foto, sem dado sensível) tinham o mesmo gap por simetria com `foto_url` — corrigido junto, por ser trivial.

Corrigido: as 4 colunas (`responsavel_nome`, `responsavel_cpf`, `responsavel_celular`, `qr_token`) entraram nas duas ramificações do trigger, mesmo padrão de `nascimento`/`cpf`.

**Testado ao vivo, com login de verdade** (diferencial desta rodada — as duas anteriores testaram só com a chave `anon`, sem sessão autenticada, porque os achados eram públicos-sem-login; este exigia estar logado pra testar de verdade):
1. Login real numa conta de teste (`rodrigo.barbosa@teste.tutti`, senha padrão `Teste123`) via `POST /auth/v1/token?grant_type=password`.
2. `PATCH /rest/v1/pessoas?id=eq.56` tentando trocar o próprio `qr_token` pra um valor escolhido e "plantar" `responsavel_nome`/`responsavel_cpf` falsos — a chamada retornou `200` (sucesso aparente), mas conferido direto no banco depois: **nada mudou**, os três campos continuaram com o valor original.
...
### 68.9 Quinta rodada (mesma sessão, pedido dela: "não vou parar até não encontrar mais nada") — mudança de método: `WITH CHECK` em vez de `USING`, achando o oposto de um vazamento

Até aqui, todas as checagens de RLS nas quatro rodadas olharam a cláusula `USING` (quem pode VER/agir sobre uma linha existente). Nunca tinha olhado `WITH CHECK` isoladamente (a cláusula que decide se um `INSERT`/`UPDATE` novo é aceito) — nas tabelas com `USING`/`WITH CHECK` iguais isso não faz diferença, mas em algumas não são o mesmo texto.

**Achado, categoria nova**: não foi vazamento de dado nem escalação de privilégio — foi o oposto, uma trava excessiva usando uma **permissão antiga, já substituída**. As três policies de `vinculos_medidas` que permitem a Diretoria inserir/editar/apagar a Medida de um Ritmista (`admin_insert_propria_bateria`, `admin_update_propria_bateria`, `admin_delete_propria_bateria`) ainda checavam `editar_ritmistas` — a capacidade ANTIGA, de antes da Reforma de Permissões (seção 62) dividir isso em `editar_instrumento_ritmista`/`editar_medidas_ritmista`. A migração da Reforma preservou a chave antiga em quem já tinha (junto da nova), então quem foi migrado continuou funcionando — mas confirmado com consulta real que **várias pessoas hoje têm `editar_medidas_ritmista=true` sem nunca ter tido `editar_ritmistas`** (foram concedidas a permissão DEPOIS da Reforma, só com a chave nova): pra essas pessoas, a tela mostra "Editar" habilitado em Medidas do Ritmista, mas o `INSERT`/`UPDATE`/`DELETE` de verdade falharia silenciosamente — exatamente o "botão aparece habilitado e não funciona" que ela já tinha marcado como inaceitável (mesma classe de bug já corrigida em 4 outras tabelas na seção 62, que passou batido nesta). As três policies também nunca checavam `tenho_acesso_ritmista_por_naipe()` — um Diretor restrito ao próprio naipe (seção 63/68.8) conseguiria mexer na Medida de um Ritmista de fora do naipe dele, diferente de toda outra ação sobre Ritmista (instrumento, declaração, repique de bossa, status).
