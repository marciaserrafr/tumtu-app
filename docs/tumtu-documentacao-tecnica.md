...
- **🔴 URGENTE (pedido da Márcia em 14/jul/2026): não é possível mudar cor da escola nem colocar logo pela interface.** Ela relatou direto: "não consigo mudar cores, não consigo colocar logo". O sistema de cor dinâmica da carteirinha já existe e funciona tecnicamente (`--cor-1/2/3/4` em `styles/carteirinha-tumtu-novo.css`, alimentado por `escolas.cor_primaria/secundaria/terciaria/quaternaria`, ver seção sobre a carteirinha) — o problema relatado é na **tela onde ela deveria conseguir editar esses valores** (`super-admin.html`, configuração de escola/bateria). Ainda não investigado nesta sessão (ela pediu pra só registrar e continuar amanhã) — próximo passo é abrir `super-admin.html` de verdade, tentar reproduzir o problema (mudar cor, subir logo) e ver o que trava: pode ser campo que não existe na tela, salvamento que falha silenciosamente, ou upload de imagem sem estar implementado de verdade.
