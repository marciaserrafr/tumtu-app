...
- **Botões da carteirinha (embaixo do cartão) — ainda não desenhado, várias situações se cruzam.** Discutido com a Márcia em 14/jul/2026, foi crescendo ao longo da conversa até virar pendência (não implementado, decisão de design ainda em aberto):
  - **Hoje** (`carteirinha.html`) a área embaixo do cartão tem "Salvar (em breve)" (nunca funcionou) e "Compartilhar" (só dispara `navigator.share` com um texto solto, não manda a carteirinha de verdade) — decisão já tomada de **remover os dois**, print/salvar imagem nativo do celular já resolve o que "Salvar"/"Compartilhar" tentavam fazer, sem precisar construir nada (ex: `html2canvas` foi cogitado e descartado por não valer o esforço).
  - **Ritmista com 1 vínculo só:** embaixo do cartão, só o botão **"Meu Perfil"** (editar dados próprios) — decisão fechada.
  - **Ritmista com 2+ vínculos (mais de uma bateria):** precisa também de um jeito de **trocar de bateria** sem digitar senha de novo — hoje isso não existe: "Sair" (`carteirinha.html`, função `sair()`) faz logout completo (`sb.auth.signOut()`), forçando login de novo pra ver outra bateria. A ideia da Márcia é esse botão de trocar ficar num lugar parecido com o "Sair" (visível, mesmo nível), não escondido — e só aparecer quando a pessoa realmente tem mais de um vínculo (pra quem só tem 1, não faz sentido mostrar). Tecnicamente dá pra fazer sem pedir senha de novo (a sessão do Supabase Auth continua válida), mas exige reaproveitar a lógica que hoje só roda dentro do formulário de `login.html` (função que busca vínculos e mostra a tela de escolha) — não é uma mudança trivial de uma linha.
  - **Admin (Mestre/Diretor) pós-piloto:** ver bullet anterior — no futuro precisa também de um botão tipo "Gestão" (entra no painel administrativo), além do "Meu Perfil". Hoje, com o modo piloto ligado, Mestre/Diretor loga e cai direto na carteirinha, igual a um ritmista comum — não acessa `admin.html` nesse momento (comportamento correto, mantém enquanto durar o piloto).
  - **Ainda em aberto:** como tudo isso convive na mesma tela quando as situações se combinam (ex: um Admin que também tem mais de uma bateria) — quantos botões cabem, qual tem mais destaque visual, etc. Motivou essa conversa toda: hoje a aba Diretoria (dentro de `admin.html`) tem um botão "Ver carteirinha ↗" que abre a carteirinha de **qualquer** Mestre/Diretor da lista (não só a própria), o que não era a intenção original da Márcia — mas o ajuste desse botão específico fica pra quando esse desenho geral for fechado, não antes.
- **Linguagem visual "modal/dialog" nas telas de formulário — percepção importante da Márcia em 14/jul/2026, ainda não desenhado nem implementado.** Ela percebeu (testando o link de preview da carteirinha, tentando entender por que o app "não parece um app de verdade") que várias telas usam o mesmo padrão: fundo escuro de página inteira + uma caixa branca centralizada, cantos arredondados, sombra pesada (`.auth-container`, `styles/components.css:197` — `box-shadow: 0 20px 60px rgba(0,0,0,0.6)`), sobrando fundo escuro visível em volta por todos os lados. Confirmado no código em `login.html` e no corpo de `carteirinha.html` (mesmo esqueleto: `body{background:#12101a;display:flex;align-items:center;justify-content:center}`) — é bem provável que `cadastro.html` e outras telas repitam o padrão, ainda não auditado tela por tela. Isso lê como um modal/pop-up de desktop, não como uma tela cheia de app nativo (abrir o app deveria já cair em tela cheia, sem essa moldura). **Distinção importante que a Márcia validou:** a carteirinha em si deve **continuar** parecendo um cartão (é literalmente uma carteirinha, de propósito) — o problema é nas telas de formulário/utilidade (login, cadastro, e o que mais repetir o padrão), que deveriam preencher a tela inteira, sem esse ar de caixa flutuante. Escopo grande (linguagem visual do app inteiro, não um ajuste pontual) — quando for atacado, mostrar mockup "como está" vs "tela cheia" antes de mexer em código de verdade, seguindo a regra de sempre pedir aprovação em mudança visual.
