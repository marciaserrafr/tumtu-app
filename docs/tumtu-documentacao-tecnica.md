...
## 60. Sessão de 27/ago/2026 (continuação): Visão Geral (Menores, Convidados, plural, ordenação) publicada em preview + desenho de Temporada
...
Cache-busting: `sw.js` v322→v323 (só JS de `admin.html` mudou, sem tocar `ficha-perfil.js`/`components.css` nessa rodada).

**Publicado em branch de preview** (`preview/visao-geral-menores-convidados-plural`), **ainda não mergeado pra `main`** — a ferramenta MCP da Vercel (usada pra gerar o link de teste sem login) caiu na mesma sessão em que o Supabase caiu ontem, e continuou fora do ar hoje. Fica aguardando ela aprovar "sem ver" (like fizemos ontem em alguns casos) ou a ferramenta reconectar.
